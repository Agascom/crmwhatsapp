...
const labelId = zod_1.z.string().min(1).describe('Label id');
const chatId = zod_1.z.string().min(1).describe('Chat JID, e.g. 628123456789@c.us or <id>@g.us');
function labelTools(labels) {
    return [
        (0, tool_descriptor_1.defineTool)({
            name: 'LabelFindAll',
            description: 'List every label on the account. WhatsApp Business only. Requires the whatsapp-web.js engine — ' +
        ...
        (0, tool_descriptor_1.defineTool)({
            name: 'LabelFindOne',
            description: 'Get one label by id. Answers 404 when no label carries that id. WhatsApp Business only, and ' +
                'whatsapp-web.js only — Baileys answers 501.',
            tier: 'read',
            sessionScoped: true,
            inputSchema: zod_1.z.object({ sessionId, labelId }),
            handler: input => labels.getLabelById(input.sessionId, input.labelId),
        }),
        (0, tool_descriptor_1.defineTool)({
            name: 'LabelListChats',
            description: 'List the chats carrying a label. Use it to answer "which conversations are tagged X". ' +
        ...
        (0, tool_descriptor_1.defineTool)({
            ...
            description: 'List the labels on one chat. WhatsApp Business only, and whatsapp-web.js only — Baileys answers 501.',
            tier: 'read',
            sessionScoped: true,
            inputSchema: zod_1.z.object({ sessionId, chatId }),
            handler: input => labels.getChatLabels(input.sessionId, input.chatId),
        }),
        (0, tool_descriptor_1.defineTool)({
            name: 'LabelUpsert',
            description: 'Create or update a label. One operation on purpose: WhatsApp carries a single write keyed by the ' +
            ...
            inputSchema: zod_1.z.object({
                sessionId,
                labelId,
                name: zod_1.z.string().min(1).max(100).optional().describe('Label text; left unchanged when omitted'),
                color: zod_1.z
                    .number()
                    .int()
                    .min(0)
            ...
            handler: input => labels
                .upsertLabel(input.sessionId, input.labelId, { name: input.name, color: input.color })
                .then(() => ({ success: true })),
        }),
        (0, tool_descriptor_1.defineTool)({
            name: 'LabelDelete',
            description: 'Delete a label from the account. This removes the label itself, not its use on one chat — use ' +
        ...
        (0, tool_descriptor_1.defineTool)({
            ...
            sessionScoped: true,
            requiredRole: api_key_entity_1.ApiKeyRole.OPERATOR,
            inputSchema: zod_1.z.object({ sessionId, chatId, labelId }),
            handler: input => labels.addLabelToChat(input.sessionId, input.chatId, input.labelId).then(() => ({ success: true })),
        }),
        (0, tool_descriptor_1.defineTool)({
            name: 'LabelRemoveFromChat',
            description: 'Remove a label from a chat, leaving the label itself in place. Works on both engines. ' +
                'WhatsApp Business only.',
            tier: 'write',
            sessionScoped: true,
            requiredRole: api_key_entity_1.ApiKeyRole.OPERATOR,
            inputSchema: zod_1.z.object({ sessionId, chatId, labelId }),
            handler: input => labels.removeLabelFromChat(input.sessionId, input.chatId, input.labelId).then(() => ({ success: true })),
        }),
    ];
}
